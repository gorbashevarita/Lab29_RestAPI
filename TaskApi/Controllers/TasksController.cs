...
public class TaskController : ControllerBase {
    ...
    [HttpGet("search")]
    public ActionResult<TaskItem> Search([FromQuery] string query) {
        if (string.IsNullOrWhiteSpace(query))
            return BadRequest(new { Message = "Параметр query не может быть пустым" });

        var results = _tasks
            .Where(t => t.Title.Contains(query, StringComparison.OrdinalIgnoreCase) || t.Description.Contains(query, StringComparison.OrdinalIgnoreCase)).ToList();
        return Ok(results);
    }
